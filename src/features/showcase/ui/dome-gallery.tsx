...
import { DOME_IMAGES, type DomeImage } from '@/shared/constants/dome-images'

type DomeGalleryProps = {
  images?: DomeImage[]
  selectedStyle?: 'All' | 'classic' | 'slim fit' | 'three piece' | 'tuxedo'
  selectedCollection?: 'All' | 'spring' | 'summer' | 'fall' | 'winter'
  fit?: number
...
function buildItems(pool: DomeImage[], seg: number): ItemDef[] {
  ...
  return coords.map((c, i) => ({
    ...
    style: usedImages[i].style,
    collection: usedImages[i].collection,
  }))
}
...
export default function DomeGallery({
  ...
  useGesture(
    {
      ...
      onDrag: ({
        ...
        // detect mostly vertical gesture on mobile → allow native scroll
        if (pointerTypeRef.current === 'touch' && Math.abs(dyTotal) > Math.abs(dxTotal) * 1.3) {
          unlockScroll()
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      if (!root2) return
      root2.appendChild(animatingOverlay)
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      // skip if user is currently dragging or zooming
      if (!draggingRef.current && !openingRef.current) {
        // rotate slowly to the left over time
        const nextY = wrapAngleSigned(rotationRef.current.y - delta * 0.002) // 0.005 = speed factor
  ...
  return (
    <>
      ...
      >
        <main
          className='absolute inset-0 grid place-items-center bg-transparent overflow-hidden md:overflow-visible'
          ref={mainRef}
          style={{
            ...
            <div className='sphere' ref={sphereRef}>
              {items.map((it, i) => {
                const dimmed =
                  ...
                  (selectedCollection !== 'All' && it.collection !== selectedCollection)
                ...
                return (
                  ...
                  >
                    <button
                      className='item__image absolute block overflow-hidden bg-gray-200 cursor-pointer'
                      style={{ inset: '10px', borderRadius: 'var(--tile-radius, 12px)' }}
                      type='button'
                    >
                      <Image
                        alt={it.alt}
